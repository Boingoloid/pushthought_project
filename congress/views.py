...
class GetCongressCampaignData(View):
    API_URL = "https://congress.api.sunlightfoundation.com/legislators/locate"

    def get(self, request, zip_code, *args, **kwargs):
        self.zip_code = zip_code
        try:
            program_id = re.findall(r'\/c\/(\d+)', self.request.META['HTTP_REFERER'])[0]
        except IndexError, KeyError:
            program_id = None
        ...
        serializer = serializers.CongressCampaignSerializer(queryset, program_id=program_id, many=True)
        # serializer.is_valid()
        return JsonResponse(serializer.data, safe=False)
